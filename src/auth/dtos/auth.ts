...
export class SignUpDTO {
    @ApiProperty({ example: 'Luana Araujo', description: 'Full name of the user' })
    @IsString({ message: "Name must be a text" })
    @IsNotEmpty({ message: "Name is required" })
    @MinLength(3, { message: "Name must be at least 3 characters" })
    name: string;

    @ApiProperty({ example: 'luana@gmail.com', description: 'User email address' })
...
export class ResetPasswordDTO {
    @ApiProperty({ example: 'uuid-token', description: 'Reset token received by email' })
    @IsString({ message: "Token must be a text" })
    @IsNotEmpty({ message: "Token is required" })
    token: string;

    @ApiProperty({ example: 'newpassword123', description: 'New password with at least 8 characters' })
    @IsString({ message: "Password must be a text" })
    @IsNotEmpty({ message: "Password is required" })
    @MinLength(8, { message: "Password must be at least 8 characters" })
    password: string;
